...
public class TypeMapper implements Mapper<TypeModel, Tree> {
	@Nonnull
	@Override
	public TypeModel map(@Nonnull MappingContext context, @Nonnull RangeExtractor extractor, @Nonnull Tree tree) {
		...
		return mapImpl(context, extractor, tree, range);
	}

	@Nonnull
	private TypeModel mapImpl(@Nonnull MappingContext context, @Nonnull RangeExtractor extractor, @Nonnull Tree tree, @Nonnull Range range) {
		return switch (tree) {
			...
			case ArrayTypeTree arrayType -> {
				TypeModel elementType = map(context, extractor, arrayType.getType());
				yield new Array(range, elementType);
			}
			case ParameterizedTypeTree parameterizedType -> {
				TypeModel identifier = map(context, extractor, parameterizedType.getType());
				List<TypeModel> typeParameters = parameterizedType.getTypeArguments().stream()
						.map(t -> map(context, extractor, t))
						.toList();
			...
			case WildcardTree wildcardTree -> {
				...
				Model boundModel = wildcardTree.getBound() == null ? null : map(context, extractor, wildcardTree.getBound());
			...
			case AnnotatedTypeTree annotatedTypeTree -> {
				...
				TypeModel type = mapImpl(context, extractor, annotatedTypeTree.getUnderlyingType(), underlyingRange);
